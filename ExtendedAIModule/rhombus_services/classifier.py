...
def classify_image(yolo_net: cv2.dnn_Net, coco_classes: list[str], layer_names: list[str], file_path: str, timestamp: int, confidence_threshold: float = 0.7) -> tuple[list[BoundingBox], Vec2]:
    ...
    for i in indices.flatten():
        ...
        box = BoundingBox(label=coco_classes[_classIDs[i]], position=Vec2(x, y), dimensions=Vec2(w, h), timestamp=timestamp);
        boxes.append(box);
...
def classify_directory(yolo_net: cv2.dnn_Net, coco_classes: list[str], directory: str, start_time: int, duration: int) -> list[FootageBoundingBoxType]:
