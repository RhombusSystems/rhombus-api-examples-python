...
def rhombus_finalizer(api_client: rapi.ApiClient, camera_uuid: str, bounding_boxes: list[rapi.FootageBoundingBoxType]) -> None:
    ...
    response = api.create_footage_bounding_boxes(body=rapi.CameraCreateFootageBoundingBoxesWSRequest(camera_uuid=camera_uuid, footage_bounding_boxes=bounding_boxes));
    ...
    response = api.create_footage_seekpoints(body=rapi.CameraCreateFootageSeekpointsWSRequest(camera_uuid=camera_uuid, footage_seek_points=seekpoints));
